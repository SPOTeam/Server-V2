...
public class ManagePostService {
  ...
  public void updatePost(Long postId, ManagePostRequest request, Long writerId,
    ...
    int updated = postRepository.updatePost(postId, request.title(), request.content(),
        request.postType(),
        writerId);
    if (updated <= 0) {
  ...
  private Post createAndSavePost(ManagePostRequest request, WriterInfo writerInfo) {
    Post post = Post.of(idGenerator.nextId(), writerInfo, request.title(), request.content(),
        request.postType());
  ...
  private void updatePostImage(Long postId, MultipartFile imageFiles) {
    ...
    PostImage postImage = PostImage.of(postId, upload.url());
