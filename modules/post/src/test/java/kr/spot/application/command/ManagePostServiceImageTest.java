...
class ManagePostServiceImageTest {
  ...
  void should_create_post_with_image_successfully() {
    ...
    WriterInfoResponse writerInfoResponse = new WriterInfoResponse(WRITER_ID,
    ...
    when(postStatsRepository.save(any(PostStats.class))).thenAnswer(
        invocation -> invocation.getArgument(0));
    when(postImageRepository.save(any(PostImage.class))).thenAnswer(
        invocation -> invocation.getArgument(0));
  ...
  void should_update_post_with_image_successfully() {
    // given
    ManagePostRequest request = new ManagePostRequest("Updated Title", "Updated Content",
        PostType.FREE_TALK);
    ...
    when(postRepository.updatePost(anyLong(), anyString(), anyString(), any(PostType.class),
        anyLong())).thenReturn(
        1);
    when(fileStoragePort.upload(any(MultipartFile.class), anyString())).thenReturn(uploadResult);
    when(postImageRepository.save(any(PostImage.class))).thenAnswer(
    ...
    // then
    verify(postRepository).updatePost(POST_ID, request.title(), request.content(),
        request.postType(), WRITER_ID);
    verify(fileStoragePort).upload(imageFile, "posts/images/");
